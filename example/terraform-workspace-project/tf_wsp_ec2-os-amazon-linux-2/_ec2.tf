...
locals {
  resources_tags = {
    ...
    ResourceGroup = "${var.environment[local.env]}-EC2-AMAZON-LINUX-2"
  }

  ebs_tags = {
    Name          = "${var.instance_name}-${var.env[local.env]}",
    ResourceGroup = "${var.environment[local.env]}-EBS-AMAZON-LINUX-2"
  }

  eip_tags = {
    Name          = "${var.instance_name}-${var.env[local.env]}",
    ResourceGroup = "${var.environment[local.env]}-EIP-AMAZON-LINUX-2"
  }
}
