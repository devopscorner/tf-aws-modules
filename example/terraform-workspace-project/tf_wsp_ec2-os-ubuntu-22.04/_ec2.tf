...
locals {
  resources_tags = {
    ...
    ResourceGroup = "${var.environment[local.env]}-EC2-UBUNTU-22.04"
  }

  ebs_tags = {
    Name          = "${var.instance_name}-${var.env[local.env]}",
    ResourceGroup = "${var.environment[local.env]}-EBS-UBUNTU-22.04"
  }

  eip_tags = {
    Name          = "${var.instance_name}-${var.env[local.env]}",
    ResourceGroup = "${var.environment[local.env]}-EIP-UBUNTU-22.04"
  }
}
