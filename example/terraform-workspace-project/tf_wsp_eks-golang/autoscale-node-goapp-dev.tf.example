# ==========================================================================
#  EKS: autoscale-node-goapp-dev.tf (EKS Autoscale Configuration)
# --------------------------------------------------------------------------
#  Description
# --------------------------------------------------------------------------
#    - Node VPC Subnet
...
## Scale Down
resource "aws_autoscaling_schedule" "scale_down_goapp_dev" {
  autoscaling_group_name = aws_eks_node_group.goapp["dev"].resources[0].autoscaling_groups[0].name
  desired_capacity       = 0
  max_size               = 0
  min_size               = 0
  recurrence             = "0 13,16 * * *"
...
## Scale Up
resource "aws_autoscaling_schedule" "scale_up_goapp_dev" {
  autoscaling_group_name = aws_eks_node_group.goapp["dev"].resources[0].autoscaling_groups[0].name
  desired_capacity       = 1
  max_size               = 5
  min_size               = 1
...
resource "aws_autoscaling_group_tag" "Clustername_group_tag_goapp_dev" {
  for_each = toset(
    [for asg in flatten(
      [for resources in aws_eks_node_group.goapp["dev"].resources : resources.autoscaling_groups]
    ) : asg.name]
...
resource "aws_autoscaling_group_tag" "Autoscaler_ownership_group_tag_goapp_dev" {
  for_each = toset(
    [for asg in flatten(
      [for resources in aws_eks_node_group.goapp["dev"].resources : resources.autoscaling_groups]
    ) : asg.name]
...
resource "aws_autoscaling_group_tag" "Autoscaler_enable_group_tag_goapp_dev" {
  for_each = toset(
    [for asg in flatten(
      [for resources in aws_eks_node_group.goapp["dev"].resources : resources.autoscaling_groups]
    ) : asg.name]
...
resource "aws_autoscaling_group_tag" "Terraform_group_tag_goapp_dev" {
  for_each = toset(
    [for asg in flatten(
      [for resources in aws_eks_node_group.goapp["dev"].resources : resources.autoscaling_groups]
    ) : asg.name]
...
# --------------------------------------------------------------------------
## Scale Down ##
output "eks_node_scale_down_goapp_dev" {
  value = aws_autoscaling_schedule.scale_down_goapp_dev.arn
}

## Scale Up ##
output "eks_node_scale_up_goapp_dev" {
  value = aws_autoscaling_schedule.scale_up_goapp_dev.arn
}
