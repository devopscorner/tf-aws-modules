# ==========================================================================
#  EKS: autoscale-node-laravel-uat.tf (EKS Autoscale Configuration)
# --------------------------------------------------------------------------
#  Description
# --------------------------------------------------------------------------
#    - Node VPC Subnet
...
locals {
  ...
  Name_laravel_uat            = "EKS-1.27-LARAVEL-UAT"
...
## Scale Down
resource "aws_autoscaling_schedule" "scale_down_laravel_uat" {
  autoscaling_group_name = aws_eks_node_group.laravel["uat"].resources[0].autoscaling_groups[0].name
  desired_capacity       = 0
  max_size               = 0
  min_size               = 0
  recurrence             = "0 13,16 * * *"
...
## Scale Up
resource "aws_autoscaling_schedule" "scale_up_laravel_uat" {
  autoscaling_group_name = aws_eks_node_group.laravel["uat"].resources[0].autoscaling_groups[0].name
  desired_capacity       = 1
  max_size               = 5
  min_size               = 1
...
resource "aws_autoscaling_group_tag" "Clustername_group_tag_laravel_uat" {
  for_each = toset(
    [for asg in flatten(
      [for resources in aws_eks_node_group.laravel["uat"].resources : resources.autoscaling_groups]
    ) : asg.name]
...
resource "aws_autoscaling_group_tag" "Autoscaler_ownership_group_tag_laravel_uat" {
  for_each = toset(
    [for asg in flatten(
      [for resources in aws_eks_node_group.laravel["uat"].resources : resources.autoscaling_groups]
    ) : asg.name]
...
resource "aws_autoscaling_group_tag" "Autoscaler_enable_group_tag_laravel_uat" {
  for_each = toset(
    [for asg in flatten(
      [for resources in aws_eks_node_group.laravel["uat"].resources : resources.autoscaling_groups]
    ) : asg.name]
...
resource "aws_autoscaling_group_tag" "Terraform_group_tag_laravel_uat" {
  for_each = toset(
    [for asg in flatten(
      [for resources in aws_eks_node_group.laravel["uat"].resources : resources.autoscaling_groups]
    ) : asg.name]
...
# --------------------------------------------------------------------------
## Scale Down ##
output "eks_node_scale_down_laravel_uat" {
  value = aws_autoscaling_schedule.scale_down_laravel_uat.arn
}

## Scale Up ##
output "eks_node_scale_up_laravel_uat" {
  value = aws_autoscaling_schedule.scale_up_laravel_uat.arn
}
